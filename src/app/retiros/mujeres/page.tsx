...
export const metadata: Metadata = {
  title: "Retiros de Pádel Solo para Mujeres | Pulso Pádel",
  description: "Retiros de pádel exclusivos para mujeres en Chile y LATAM. Entrenamiento, comunidad y experiencias únicas.",
};

function formatPrice(n: number) { return "$" + n.toLocaleString("es-CL"); }

export default async function RetirosMujeresPage() {
  ...
  }).from(retiros).where(and(eq(retiros.estado, "publicado"), eq(retiros.genero, "mujeres")));
  ...
  return (
    <>
      ...
      <main>
        <section className="px-6 md:px-8 py-12 border-b border-line" style={{ background: "linear-gradient(135deg, #f5f0ff 0%, #fdf4ff 100%)" }}>
          <div className="max-w-[1400px] mx-auto">
            <div className="mono text-xs text-[#7C3AED] uppercase tracking-widest mb-4">← <Link href="/retiros" className="hover:underline">Retiros</Link> · Solo mujeres</div>
            <h1 className="display text-5xl md:text-6xl font-semibold" style={{ letterSpacing: "-0.03em" }}>
              Retiros <span className="italic font-normal text-[#7C3AED]">femeninos</span>
            </h1>
            <p className="text-ink-muted text-xl mt-4 max-w-2xl leading-relaxed">
              Espacios creados por y para mujeres que aman el pádel. Entrenamiento de calidad, comunidad real y destinos increíbles.
            </p>
          </div>
        </section>
        <section className="px-6 md:px-8 py-10 max-w-[1400px] mx-auto">
          {data.length === 0 ? (
            <div className="text-center py-20 text-ink-muted">
              ...
              <p className="text-sm mt-2">Estamos sumando retiros femeninos. <Link href="/retiros" className="text-[#7C3AED] hover:underline">Ver todos los retiros →</Link></p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {data.map(r => (
                <Link key={r.slug} href={`/retiros/${r.slug}`} className="border border-line rounded-xl overflow-hidden bg-canvas hover:border-[#7C3AED]/50 transition-colors">
                  <div className="aspect-[16/9] bg-gradient-to-br from-[#f5f0ff] to-[#fdf4ff] flex items-center justify-center text-4xl">
                    {paisFlag[r.pais] ?? "🌎"}
                  </div>
                  <div className="p-5">
